  ...
  return (
    <>
      ...
      <dl className="grid grid-cols-1 gap-1 overflow-hidden text-center sm:grid-cols-1 lg:grid-cols-3 pb-4">
        <header className="flex items-end justify-start py-2 lg:flex-none">
                      ...
                      {day.events.map((event) => (
                        <li key={event.id}>
                          <a onClick={() => editItem(event)} className="cursor-pointer group flex items-center">
                            <p className="flex-auto truncate font-medium text-[10px] text-pink-600 group-hover:text-pink-500 rounded-full bg-pink-50 ring-1 ring-inset ring-pink-600/10 my-1 px-2 py-0.5">
                              {event.name}
                            </p>
                            {
