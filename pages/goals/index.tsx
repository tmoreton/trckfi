          ...
          return(
            <li key={goal.id} className="flex flex-col gap-5 lg:gap-10 pt-12 sm:flex-row">
              ...
              <div className="flex-auto">
                <h3 className="text-xl lg:text-3xl font-bold leading-8 tracking-tight text-gray-900">{goal.name}</h3>
                <p className="text-md lg:text-lg leading-7 text-gray-600">{DateTime.fromISO(goal.date).toLocaleString(DateTime.DATE_FULL)}</p>
                <div className="flex justify-between mt-6">
                  <div className="block">
                    <p className="text-xs">Current Amount:</p>
                    <p className="text-xl lg:text-2xl font-bold leading-7 text-green-600 mb-4">{commaShort(goal?.account?.details?.current - Number(goal.initial_amount))}</p>
                  </div>
                  <div className="block">
                    <p className="text-xs">Goal Amount:</p>
                    <p className="text-xl lg:text-2xl font-bold leading-7 text-gray-700 mb-4">{commaShort(goal.amount)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-7 lg:grid-cols-3 mt-2">
                  <div className="col-span-3 lg:col-span-1 flex items-center">
                    ...
                    <div>
                      <p className="ml-4 text-xs font-bold trunc">{goal?.account?.name}</p>
                      <p className="ml-4 text-xs">{goal?.account?.official_name}</p>
                    </div>
                  </div>
                  <div className="col-span-3 lg:col-span-1 text-center">
                    <p className="text-xs text-gray-500">Last Updated:</p>
                    <p className="text-xs text-gray-500">{DateTime.fromISO(goal?.account?.updated_at).toLocaleString(DateTime.DATE_FULL)}</p>
                  </div>
